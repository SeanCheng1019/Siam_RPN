    ...
    def __getitem__(self, idx):  # 何时调用的，何时传入的index参数
        ...
        for index in all_idx:
            ...
            for id in range(len(instance_indexes)):
                ...
                instance_gt_w, instance_gt_h, exemplar_img_w, exemplar_img_h = get_wh_from_img_path(
                    instance_whole_path_names[id])
                instance_gt_ws.append(exemplar_gt_w)
                instance_gt_hs.append(exemplar_gt_h)
                instance_img_ws.append(exemplar_img_w)
                instance_img_hs.append(exemplar_img_h)
                # 选到模版图片 (511,511)
            ...
            for img in instance_imgs:
                ...
                img = crop_and_pad(img, (img.shape[0] - 1) / 2,
            ...
            # 返回放缩后的图片，放缩后的目标宽高
            instance_img = instance_his_imgs[-1]
